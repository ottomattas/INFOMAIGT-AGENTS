...
class Environment:
    def __init__(self):
        ...
        self.winner = None
        self.ended = False
        self.num_states = 3**(g.board.size*g.board.size)
    ...
    def game_over(self, force_recalculate=False):
        ...
        if (g.victory(self, hint, playerid) == True): return True
...
#     for i in xrange(g.board.size):
#       for j in xrange(g.board.size):
#         if env.is_empty(i, j):
#           env.board[i,j] = sym
